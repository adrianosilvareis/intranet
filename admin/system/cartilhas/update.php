    ...
    <article>
        ...
        if (isset($post) && $post['SendPostForm']):
            ...
            if (!$WsPosts->Execute()->getResult()):
                header('Location: painel.php?exe=cartilhas/index&empty=true');
            else:
        ...
        if ($checkCreate && empty($cadastra)):
            WSErro("A Cartilha <b>{$post['post_title']}</b> foi cadastrado com sucesso no sistema!", WS_ACCEPT);
        endif;
        ?>
